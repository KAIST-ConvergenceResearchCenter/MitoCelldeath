...
data_dir = 'D:\\Dataset\\Mitochondria_v4\\'
...
def dump_pickle_object(dump_object, file_name, compress=True, compress_level=9):
    ...
    if compress:
        pkl_write(file_name, gzip.compress(data, compresslevel=compress_level))
    else:
        pkl_write(file_name, data)
...
def dump_json_object(dump_object, file_name, compress=False, indent=4):
    data = json.dumps(
        dump_object, cls=NumpyAwareJSONEncoder, sort_keys=False, indent=indent)
    if compress:
...
def pkl_write(file_name, data, mode='wb'):
    file_name = os.path.join(data_dir + file_name)
    with open(file_name, mode) as f:
        f.write(data)
